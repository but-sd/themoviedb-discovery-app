...
export default function TvItemCard({ show }: TvItemCardProps) {
  ...
  return (
    <Link className="item-card-link" to={`/tv/${show.id}`} aria-label={`Open details for ${show.name}`}>
      <article className="item-card">
        {posterSrc ? (
          <img src={posterSrc} alt={`Poster for ${show.name}`} className="movie-poster" />
        ) : (
        ...
        <div className="movie-copy">
          <h2>{show.name}</h2>
          <p className="movie-meta">{firstAirYear} · Rating {show.vote_average.toFixed(1)}</p>
        </div>
      </article>
    </Link>
  )
